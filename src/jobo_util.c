...
const char *player_class_name( CHAR_DATA *ch )
{
  ...
  if (IS_SET(ch->class, CLASS_DEMON))        return "Demon";
  if (IS_SET(ch->class, CLASS_MAGE))         return "Mage";
  if (IS_SET(ch->class, CLASS_WEREWOLF))     return "Werewolf";
  if (IS_SET(ch->class, CLASS_VAMPIRE))      return "Vampire";
  if (IS_SET(ch->class, CLASS_SAMURAI))      return "Samurai";
  if (IS_SET(ch->class, CLASS_DROW))         return "Drow";
  if (IS_SET(ch->class, CLASS_MONK))         return "Monk";
  if (IS_SET(ch->class, CLASS_NINJA))        return "Ninja";
  if (IS_SET(ch->class, CLASS_LICH))         return "Lich";
  if (IS_SET(ch->class, CLASS_SHAPESHIFTER)) return "Shapeshifter";
  if (IS_SET(ch->class, CLASS_TANARRI))      return "Tanarri";
  if (IS_SET(ch->class, CLASS_ANGEL))        return "Angel";
  if (IS_SET(ch->class, CLASS_UNDEAD_KNIGHT))return "Undead Knight";
  if (IS_SET(ch->class, CLASS_DROID))        return "Droid";
  return "Human";
}
