...
- **Tiger** (`shift tiger`): Focuses heavily on high physical damage at the cost of armor.
  ...
  - *Bot Behavior:* Used as the primary combat state for grinding mobs due to raw offensive power.
- **Hydra** (`shift hydra`): Balanced form with a powerful breath weapon.
  - Can use `breath` attack on opponents. The frequency and damage escalate as Hydra format is leveled up.
- **Bull** (`shift bull`): Brutal physical form excelling in stuns and damage spikes.
  ...
  - *Bot Behavior:* Used as the primary form for Player vs Player (PvP) combat to disable opponents.
